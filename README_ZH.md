...
| 界面 | 说明 |
|------|------|
| ![主界面](assets/readme/01-main-workspace.png) | **主工作区** — 助理栏、历史任务、会话标题栏（新任务、工作区）与当前会话对话区。 |
| ![团队管理](assets/readme/02-team-management.png) | **团队管理** — 创建与编辑助理、人设与模型、技能与快捷入口（MCP、知识库、SOP、计划表）。 |
| ![消息通道 — 微信](assets/readme/03-messaging-channels-weixin.png) | **消息通道** — 在「机器人对话」中为助理绑定钉钉、飞书、Telegram、QQ、企业微信、Discord、**微信**等平台；支持各平台凭证、白名单与连接状态。**微信**支持扫码登录个人号，用于私聊收发与策略配置。 |
| ![技能商店](assets/readme/04-skill-store.png) | **技能商店** — 浏览与管理技能，支持搜索、分类与安装/移除。 |
| ![日历](assets/readme/05-scheduler.png) | **日历 / 调度** — 定时任务、Hook 与自动化（单次、间隔、每日、心跳等）。 |
| ![设置](assets/readme/06-settings.png) | **设置** — 账号与模型、内嵌 API、机器人、个性化、更新与诊断。 |
